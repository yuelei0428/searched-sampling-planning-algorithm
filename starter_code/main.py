...
import Planner
from Planner import collision_check
...
def runtest(mapfile, start, goal, verbose = True):
  ...
  # Part 1: Check for collisions
  collision = False
  for i in range(len(path)-1):
    collision = collision_check(path[i], path[i+1], blocks)
    if collision == True:
      break


  goal_reached = np.linalg.norm(path[-1] - goal) <= 0.5 # 0.5 is stepsize
  print('Goal reached: %r'%goal_reached)
...
if __name__=="__main__":
  test_single_cube()
  plt.savefig('result_Astar/single_cube.png')
  test_maze()
  plt.savefig('result_Astar/maze.png')
  test_flappy_bird()
  plt.savefig('result_Astar/flappy_bird.png')
  test_monza()
  plt.savefig('result_Astar/monza.png')
  test_window()
  plt.savefig('result_Astar/window.png')
  test_tower()
  plt.savefig('result_Astar/tower.png')
  test_room()
  plt.savefig('result_Astar/room.png')
  plt.show(block=True)
